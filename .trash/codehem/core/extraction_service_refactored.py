"""
Extraction service module, refactored to use the PostProcessorFactory.

This module provides the ExtractionService class, which is responsible for
extracting code elements from source code using language-specific extractors
and post-processors.
"""
...
class ExtractionService:
    ...
    def __init__(self, language_code: str):
        ...
        try:
            # Use the PostProcessorFactory to get the appropriate post-processor
            self.post_processor = PostProcessorFactory.get_post_processor(language_code)
            logger.debug(f"Initialized post-processor {self.post_processor.__class__.__name__} for {language_code}")
        except (ValueError, Exception) as e:
            logger.error(f"Failed to get post-processor for {language_code}: {e}", exc_info=True)
    ...
    def extract_all(self, code: str) -> 'CodeElementsResult':
        ...
        logger.info(f'ExtractionService: Starting full extraction and post-processing for {self.language_code}')
        ...
        try:
            ...
            logger.debug(f"ExtractionService: Using post-processor: {self.post_processor.__class__.__name__}")
            
            # Use the post_processor.process_all method to handle the raw elements
            result = self.post_processor.process_all(raw_elements)
            
            # Log final element count
            logger.info(f'ExtractionService: Completed full extraction for {self.language_code}. Top-level element count: {len(result.elements)}')
