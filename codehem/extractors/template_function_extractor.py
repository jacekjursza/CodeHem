...
class TemplateFunctionExtractor(TemplateExtractor):
        ...
        for node, capture_name in query_results:
            if capture_name == 'function_def':
                ...
                if name_node:
                    ...
                    # Get parameters
                    parameters = ExtractorHelpers.extract_parameters(
                        ast_handler, function_def, code_bytes, is_self_or_this=False
                    )
                    
                    # Get return information
                    return_info = ExtractorHelpers.extract_return_info(
                        ast_handler, function_def, code_bytes
                    )
                    ...
                    function_info = {
                        ...
                        'content': content,
                        'range': {
                            'start': {'line': function_def.start_point[0] + 1, 'column': function_def.start_point[1]},
                            'end': {'line': function_def.end_point[0] + 1, 'column': function_def.end_point[1]}
                        },
                ...
                if function_node:
                    ...
                    # Get parameters
                    parameters = ExtractorHelpers.extract_parameters(
                        ast_handler, function_node, code_bytes, is_self_or_this=False
                    )
                    
                    # Get return information
                    return_info = ExtractorHelpers.extract_return_info(
                        ast_handler, function_node, code_bytes
                    )
                    ...
                    function_info = {
                        ...
                        'content': content,
                        'range': {
                            'start': {'line': function_node.start_point[0] + 1, 'column': function_node.start_point[1]},
                            'end': {'line': function_node.end_point[0] + 1, 'column': function_node.end_point[1]}
                        },
                        'parameters': parameters,
                        'return_info': return_info
                    }
    ...
    def _extract_parameters_regex(self, content):
        ...
        if param_match:
            ...
            for param in param_list:
                ...
                # Handle Python-style type annotations
                if ':' in param:
                    name_part, type_part = param.split(':', 1)
                    param_dict['name'] = name_part.strip()
                    param_dict['type'] = type_part.strip()
                
                # Handle TypeScript/JavaScript parameter type
                if param.startswith('...'):  # Rest parameter
    ...
    def _extract_return_info_regex(self, content):
        ...
        # Try to get explicit return type annotation (Python)
        ...
        # Try to get TypeScript return type
