# Note: Replacing the whole file content.
# Main change: Drastically simplified static_property query.
...
import logging
import sys # Keep for debug prints if needed
...
class ElementTypeTemplate:
    ...
    def for_language(self, language_code: str) -> Optional[Dict[str, Any]]:
        ...
        if tree_sitter_query and placeholders:
            import re
            placeholders_in_pattern = set(re.findall('\\{([^{}]+)\\}', tree_sitter_query))
        ...
        if regexp and placeholders:
            import re
            placeholders_in_pattern = set(re.findall('\\{([^{}]+)\\}', regexp))
            missing_keys = placeholders_in_pattern - set(placeholders.keys())
            if missing_keys:
        ...
        if ts_ok and rx_ok:
            ...
            return {'language_code': language_code, 'element_type': self.element_type, 'tree_sitter_query': final_ts_query, 'regexp_pattern': final_regexp, 'custom_extract': self.custom_extract}
        else:
...
CLASS_TEMPLATE = ElementTypeTemplate(
    ...
    regexp_pattern=r'{CLASS_PATTERN}\s+({IDENTIFIER_PATTERN})(?:{INHERITANCE_PATTERN})?\s*{BODY_START}',
    placeholder_map={
        'python': {
            'CLASS_NODE': 'class_definition', 'NAME_NODE': 'identifier', 'BODY_NODE': 'block',
            'CLASS_PATTERN': 'class', 'IDENTIFIER_PATTERN': r'[a-zA-Z_][a-zA-Z0-9_]*',
            'INHERITANCE_PATTERN': r'\s*\([^)]*\)', 'BODY_START': ':'
        },
        'typescript': {
            'CLASS_NODE': 'class_declaration', 'NAME_NODE': 'type_identifier', 'BODY_NODE': 'class_body',
            'CLASS_PATTERN': 'class', 'IDENTIFIER_PATTERN': r'[a-zA-Z_][a-zA-Z0-9_]*',
            'INHERITANCE_PATTERN': r'(?:\s+extends\s+[a-zA-Z_][a-zA-Z0-9_]*)?(?:\s+implements\s+[^{]+)?',
            'BODY_START': r'\s*\{'
        }
    }
)
...
METHOD_TEMPLATE = ElementTypeTemplate(
    ...
    placeholder_map={
        'python': {
            'NAME_NODE': 'identifier', 'FIRST_PARAM_ID': 'self',
            'METHOD_PATTERN': 'def', 'IDENTIFIER_PATTERN': r'[a-zA-Z_][a-zA-Z0-9_]*',
            'RETURN_TYPE_PATTERN': r'(?:\s*->.*?)?', 'BODY_START': ':'
        },
        'typescript': {
             'NAME_NODE': 'property_identifier', 'FIRST_PARAM_ID': 'this',
             'METHOD_PATTERN': '', 'IDENTIFIER_PATTERN': r'[a-zA-Z_][a-zA-Z0-9_]*',
             'RETURN_TYPE_PATTERN': r'(?:\s*:[^{;]+)?', 'BODY_START': r'\s*\{'
        }
    }
)
...
FUNCTION_TEMPLATE = ElementTypeTemplate(
    ...
    placeholder_map={
        'python': {
             ...
             'FUNCTION_PATTERN': 'def', 'IDENTIFIER_PATTERN': r'[a-zA-Z_][a-zA-Z0-9_]*',
             'PARAMS_PATTERN': r'(?!self\s*[,)])[^)]*',
             'RETURN_TYPE_PATTERN': r'(?:\s*->.*?)?', 'BODY_START': ':'
        },
         'typescript': {
             'NAME_NODE': 'identifier',
             'FUNCTION_PATTERN': 'function', 'IDENTIFIER_PATTERN': r'[a-zA-Z_][a-zA-Z0-9_]*',
             'PARAMS_PATTERN': r'[^)]*',
             'RETURN_TYPE_PATTERN': r'(?:\s*:[^{;]+)?', 'BODY_START': r'\s*\{'
         }
    }
...
IMPORT_TEMPLATE = ElementTypeTemplate(
    ...
    # Keep simple separate captures query - verified in debug script
    ...
    placeholder_map={
        ...
        'typescript': {
            'IMPORT_NODE': '(import_statement) @import',
...
PROPERTY_GETTER_TEMPLATE = ElementTypeTemplate(
    ...
    """,
    regexp_pattern=r'{GETTER_DEcorATOR_PATTERN}\s*\n\s*{METHOD_PATTERN}\s+({IDENTIFIER_PATTERN})\s*\(\s*{FIRST_PARAM_PATTERN}[^)]*\){RETURN_TYPE_PATTERN}\s*:{BODY_CAPTURE_LOOKAHEAD}',
    placeholder_map={
        'python': {
            'NAME_NODE': 'identifier', 'GETTER_DECORATOR_ID': 'property',
            'GETTER_DECORATOR_PATTERN': '@property', 'METHOD_PATTERN': 'def',
            'IDENTIFIER_PATTERN': r'[a-zA-Z_][a-zA-Z0-9_]*', 'FIRST_PARAM_PATTERN': 'self',
            'RETURN_TYPE_PATTERN': r'(?:\s*->.*?)?',
            'BODY_CAPTURE_LOOKAHEAD': r'(.*?)(?=\n(?:[ \t]*@|[ \t]*def|\Z))'
        }
    }
)
...
PROPERTY_SETTER_TEMPLATE = ElementTypeTemplate(
    ...
    """,
    regexp_pattern=r'@{PROPERTY_NAME_PATTERN}\.{SETTER_ATTR_PATTERN}\s*\n\s*{METHOD_PATTERN}\s+({IDENTIFIER_PATTERN})\s*\(\s*{FIRST_PARAM_PATTERN}[^)]*\){RETURN_TYPE_PATTERN}\s*:{BODY_CAPTURE_LOOKAHEAD}',
    placeholder_map={
        'python': {
             'NAME_NODE': 'identifier', 'SETTER_DECORATOR_ATTR': 'setter',
             'PROPERTY_NAME_PATTERN': r'([a-zA-Z_][a-zA-Z0-9_]*)', 'SETTER_ATTR_PATTERN': 'setter',
             'METHOD_PATTERN': 'def', 'IDENTIFIER_PATTERN': r'[a-zA-Z_][a-zA-Z0-9_]*',
             'FIRST_PARAM_PATTERN': 'self', 'RETURN_TYPE_PATTERN': r'(?:\s*->.*?)?',
             'BODY_CAPTURE_LOOKAHEAD': r'(.*?)(?=\n(?:[ \t]*@|[ \t]*def|\Z))'
        }
    }
)

# --- MODIFIED STATIC_PROPERTY_TEMPLATE ---
STATIC_PROPERTY_TEMPLATE = ElementTypeTemplate(
    ...
    # Simplest possible query - just find assignments anywhere.
    # Filtering for class scope must happen in extractor code.
    tree_sitter_pattern="""
    (assignment left: (identifier) @prop_name) @static_prop_assign
    (typed_assignment left: (identifier) @prop_name) @static_prop_assign_typed
    """,
    regexp_pattern=r'{OPTIONAL_NEWLINE_INDENT}({IDENTIFIER_PATTERN}){OPTIONAL_TYPE_HINT}\s*=\s*({VALUE_CAPTURE}){OPTIONAL_COMMENT_ENDLINE}',
    placeholder_map={
        'python': {
            # Regex placeholders remain the same
            'IDENTIFIER_PATTERN': r'[a-zA-Z_][a-zA-Z0-9_]*',
            'OPTIONAL_NEWLINE_INDENT': r'(?:^|\n)\s+',
            'OPTIONAL_TYPE_HINT': r'(?:\s*:\s*[^=]+?)?',
            'VALUE_CAPTURE': r'.+?',
            'OPTIONAL_COMMENT_ENDLINE': r'(?:$|\s*#)'
        }
    }
)
# --- END MODIFIED STATIC_PROPERTY_TEMPLATE ---
...
DECORATOR_TEMPLATE = ElementTypeTemplate(
    ...
    regexp_pattern=r'{DECORATOR_PREFIX}({QUALIFIED_NAME_PATTERN})(?:{ARGS_PATTERN})?',
    placeholder_map={
        'python': {
            'DECORATOR_PREFIX': '@',
...
INTERFACE_TEMPLATE = ElementTypeTemplate(
    ...
    regexp_pattern=r'{INTERFACE_PATTERN}\s+([a-zA-Z_][a-zA-Z0-9_]*)(?:{EXTENDS_PATTERN})?\s*{BODY_START}',
    placeholder_map={
        'typescript': {
            'INTERFACE_NODE': 'interface_declaration', 'NAME_NODE': 'type_identifier', 'BODY_NODE': 'object_type',
            'INTERFACE_PATTERN': 'interface', 'EXTENDS_PATTERN': r'(?:\s+extends\s+[^{]+)?', 'BODY_START': r'\s*\{'
        }
    }
)
...
TYPE_ALIAS_TEMPLATE = ElementTypeTemplate(
    ...
    regexp_pattern=r'{TYPE_PATTERN}\s+([a-zA-Z_][a-zA-Z0-9_]*)\s*=',
    placeholder_map={
        'typescript': {
            'TYPE_NODE': 'type_alias_declaration', 'NAME_NODE': 'type_identifier',
            'TYPE_PATTERN': 'type'
        }
    }
)
...
ELEMENT_TYPE_TEMPLATES: Dict[CodeElementType, ElementTypeTemplate] = {
    ...
    CodeElementType.STATIC_PROPERTY: STATIC_PROPERTY_TEMPLATE, # Updated query
