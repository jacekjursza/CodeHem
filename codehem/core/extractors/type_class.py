            ...
            if node_type == 'method_name':
                ...
                if method_node:
                    ...
                    if is_method:
                        ...
                        methods.append({'type': method_type, 'name': method_name, 'content': method_content, 'class_name': class_name, 'range': {'start': {'line': start_line, 'column': method_node.start_point[1]}, 'end': {'line': end_line, 'column': method_node.end_point[1]}}, 'decorators': decorators, 'parameters': parameters, 'return_type': return_type, 'property_name': property_name})
        return methods
                        ...
                        if prop_assign_node:
                            ...
                            if left_node and left_node.type == 'attribute':
                                ...
                                if prop_name_node:
                                    ...
                                    properties.append({'type': 'property', 'name': prop_name, 'content': f'self.{prop_name} = {value}' if value else f'self.{prop_name}', 'range': {'start': {'line': prop_assign_node.start_point[0], 'column': prop_assign_node.start_point[1]}, 'end': {'line': prop_assign_node.end_point[0], 'column': prop_assign_node.end_point[1]}}, 'value': value, 'value_type': value_type})
        return properties
                    ...
                    if expr_node:
                        ...
                        if right_node:
                            ...
                            if value.isdigit():
                                ...
                                value_type = 'dict' if ':' in value else 'set'
                        static_properties.append({'type': 'static_property', 'name': prop_name, 'content': ast_handler.get_node_text(expr_node, code_bytes), 'range': {'start': {'line': expr_node.start_point[0], 'column': expr_node.start_point[1]}, 'end': {'line': expr_node.end_point[0], 'column': expr_node.end_point[1]}}, 'value': value, 'value_type': value_type, 'is_static': True})
        return static_properties
