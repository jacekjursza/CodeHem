...
class PythonExtractionService(BaseExtractionService):
    ...
    def extract_imports(self, code: str, result: CodeElementsResult) -> None:
        ...
        if imports_info:
            import_element = CodeElement(type=CodeElementType.IMPORT, name='imports', content=imports_info['content'], 
                                         range=CodeRange(start_line=imports_info['start_line'], end_line=imports_info['end_line'], node=None), 
                                         additional_data={'import_statements': imports_info.get('statements', imports_info.get('lines', []))})
    ...
    def extract_classes(self, code: str, code_bytes: bytes, result: CodeElementsResult) -> None:
        ...
        try:
            ...
            # Find both direct class definitions and decorated class definitions
            class_query = '(class_definition name: (identifier) @class_name)'
            decorated_class_query = '''
            (decorated_definition 
              decorator: (decorator)
              definition: (class_definition name: (identifier) @class_name))
            '''
            ...
            # Process direct class definitions
                ...
                if capture_name == 'class_name':
                    ...
                    if class_node:
                        ...
                        class_element = CodeElement(
                            ...
                            range=CodeRange(start_line=start_line, end_line=end_line, node=class_node),
                        ...
                        for decorator in class_decorators:
                            ...
                            meta_element = CodeElement(
                                ...
                                additional_data={'target_type': 'class', 'target_name': class_name}
                            )
                ...
                if capture_name == 'class_name':
                    ...
                    if decorated_def_node:
                        ...
                        class_element = CodeElement(
                            ...
                            name=class_name, 
                            content=class_content,
                            range=CodeRange(start_line=start_line, end_line=end_line, node=decorated_def_node),
                            additional_data={'decorators': class_decorators}
                        )
            ...
            # Fallback to regex approach if tree-sitter parsing fails
    ...
    def extract_functions(self, code: str, code_bytes: bytes, result: CodeElementsResult) -> None:
        ...
        try:
            ...
            # Find both direct function definitions and decorated function definitions
            function_query = '(function_definition name: (identifier) @func_name)'
            decorated_function_query = '''
            (decorated_definition 
              decorator: (decorator)
            ...
            # Get class names to filter out methods
            classes = self._get_class_names(code)
            
            # Process direct function definitions
                ...
                if capture_name == 'func_name':
                    ...
                    if function_node:
                        ...
                        func_element = CodeElement(
                            ...
                            range=CodeRange(start_line=start_line, end_line=end_line, node=function_node),
                        ...
                        if return_info['return_type'] or return_info['return_values']:
                            return_element = CodeElement(
                                ...
                                additional_data={'values': return_info['return_values']}
                            )
            ...
            for node, capture_name in decorated_results:
                if capture_name == 'func_name':
                    ...
                    if decorated_def_node:
                        ...
                        # Find the function definition within the decorated definition
                        function_node = None
                        for child_idx in range(decorated_def_node.named_child_count):
                            child = decorated_def_node.named_child(child_idx)
            ...
            for match in function_pattern.finditer(code):
                ...
                element_type_str = self.strategy.determine_element_type(decorators, is_method=False)
                element_type = getattr(CodeElementType, element_type_str.upper())
                func_element = CodeElement(type=element_type, name=func_name, content=func_content, 
                                          range=CodeRange(start_line=start_line, end_line=end_line, node=None), 
                                          additional_data={'decorators': decorators})
                self._add_decorator_meta_elements(parent_element=func_element, decorators=decorators, 
                                                parent_name=func_name, target_type='function', target_name=func_name)
                parameters = self.finder.get_function_parameters(func_content, func_name)
                for param in parameters:
                    param_element = CodeElement(type=CodeElementType.PARAMETER, name=param['name'], content=param['name'], 
                                              parent_name=func_name, value_type=param.get('type'), 
                                              additional_data={'optional': param.get('optional', False), 'default': param.get('default')})
                    func_element.children.append(param_element)
                return_info = self.finder.get_function_return_info(func_content, func_name)
                if return_info['return_type'] or return_info['return_values']:
                    return_element = CodeElement(type=CodeElementType.RETURN_VALUE, name=f'{func_name}_return', 
                                               content=return_info['return_type'] if return_info['return_type'] else '', 
                                               parent_name=func_name, value_type=return_info['return_type'], 
                                               additional_data={'values': return_info['return_values']})
                    func_element.children.append(return_element)
                result.elements.append(func_element)
    ...
    def _create_class_element(self, code: str, code_bytes: bytes, class_name: str, start_line: int, end_line: int, class_content: str) -> CodeElement:
        ...
        class_element = CodeElement(type=CodeElementType.CLASS, name=class_name, content=class_content, 
                                  range=CodeRange(start_line=start_line, end_line=end_line, node=None), 
                                  additional_data={'decorators': class_decorators})
        for decorator in class_decorators:
            decorator_name = self._extract_decorator_name(decorator)
            meta_element = CodeElement(type=CodeElementType.DECORATOR, name=decorator_name, content=decorator, 
                                     parent_name=class_name, additional_data={'target_type': 'class', 'target_name': class_name})
            class_element.children.append(meta_element)
    ...
    def _extract_methods(self, code: str, code_bytes: bytes, class_name: str, class_element: CodeElement) -> None:
        ...
        try:
            ...
            # Find the class node
            class_query = f'(class_definition name: (identifier) @class_name (#eq? @class_name "{class_name}"))'
            ...
            # Get methods directly from the class block
            method_query = '(function_definition name: (identifier) @method_name)'
            method_captures = self.analyzer.ast_handler.execute_query(method_query, class_node, code_bytes)
            
            # Process direct method definitions
                ...
                if capture_name == 'method_name':
                    ...
                    if method_node:
                        ...
                        method_element = CodeElement(
                            ...
                            range=CodeRange(start_line=start_line, end_line=end_line, node=method_node),
                            parent_name=class_name,
                            additional_data={'decorators': method_decorators}
                        )
            ...
            # Get decorated methods from the class block
            decorated_method_query = '''
            (decorated_definition
              decorator: (decorator)
            ...
            # Process decorated method definitions
            ...
            for match in method_pattern.finditer(class_code):
                ...
                element_type_str = self.strategy.determine_element_type(method_decorators, is_method=True)
                element_type = getattr(CodeElementType, element_type_str.upper())
                method_element = CodeElement(type=element_type, name=method_name, content=method_content, 
                                          range=CodeRange(start_line=start_line, end_line=end_line, node=None), 
                                          parent_name=class_name, additional_data={'decorators': method_decorators})
                for decorator in method_decorators:
                    decorator_name = self._extract_decorator_name(decorator)
                    decorator_element = CodeElement(type=CodeElementType.DECORATOR, name=decorator_name, content=decorator, 
                                                 parent_name=f'{class_name}.{method_name}', 
                                                 additional_data={'target_type': element_type.value, 'target_name': method_name, 'class_name': class_name})
                    method_element.children.append(decorator_element)
                parameters = self.finder.get_function_parameters(method_content, method_name, class_name)
                for param in parameters:
                    param_element = CodeElement(type=CodeElementType.PARAMETER, name=param['name'], content=param['name'], 
                                              parent_name=f'{class_name}.{method_name}', value_type=param.get('type'), 
                                              additional_data={'optional': param.get('optional', False), 'default': param.get('default')})
                    method_element.children.append(param_element)
                return_info = self.finder.get_function_return_info(method_content, method_name, class_name)
                if return_info['return_type'] or return_info['return_values']:
                    return_element = CodeElement(type=CodeElementType.RETURN_VALUE, name=f'{method_name}_return', 
                                               content=return_info['return_type'] if return_info['return_type'] else '', 
                                               parent_name=f'{class_name}.{method_name}', value_type=return_info['return_type'], 
                                               additional_data={'values': return_info['return_values']})
                    method_element.children.append(return_element)
                class_element.children.append(method_element)
    ...
    def _scan_for_properties(self, class_code: str, full_code: str, class_name: str, class_element: CodeElement, property_names: Set[str]) -> None:
        ...
        try:
            ...
            property_getter_query = '''
            (decorated_definition
              decorator: (decorator name: (identifier) @dec_name (#eq? @dec_name "property"))
              definition: (function_definition name: (identifier) @property_name))
            '''
                ...
                if capture_name == 'property_name':
                    ...
                    if decorated_def_node:
                        ...
                        property_element = CodeElement(
                            type=CodeElementType.PROPERTY_GETTER,
                            name=property_name,
                            content=content,
                            range=CodeRange(start_line=start_line, end_line=end_line, node=decorated_def_node),
                            parent_name=class_name,
            ...
            property_setter_query = '''
            (decorated_definition
              decorator: (decorator name: (attribute
                                        object: (identifier) @prop_obj
                                        attribute: (identifier) @attr (#eq? @attr "setter")))
              definition: (function_definition name: (identifier) @property_name))
            '''
            ...
            for node, capture_name in property_setter_captures:
                if capture_name == 'property_name':
                    ...
                    if decorated_def_node:
                        ...
                        property_element = CodeElement(
                            type=CodeElementType.PROPERTY_SETTER,
                            name=property_name,
                            content=content,
                            range=CodeRange(start_line=start_line, end_line=end_line, node=decorated_def_node),
                            parent_name=class_name,
                            additional_data={'decorators': decorators, 'property_obj': prop_obj_name}
                        )
            ...
            for match in property_matches:
                ...
                if start_line > 0 and end_line > 0:
                    ...
                    decorators = ['@property']
                    property_element = CodeElement(type=CodeElementType.PROPERTY_GETTER, name=property_name, content=content, 
                                                range=CodeRange(start_line=start_line, end_line=end_line, node=None), 
                                                parent_name=class_name, additional_data={'decorators': decorators})
                    self._add_decorator_meta_elements(parent_element=property_element, decorators=decorators, 
                                                    parent_name=f'{class_name}.{property_name}', 
                                                    target_type='property', target_name=property_name, class_name=class_name)
                    class_element.children.append(property_element)
    ...
    def _extract_static_properties(self, class_code: str, full_code: str, class_name: str, class_element: CodeElement) -> None:
        ...
        try:
            ...
            # Look for assignments in the class block
            static_prop_query = '''
            ...
            for node, capture_name in static_prop_captures:
                if capture_name == 'prop_name':
                    ...
                    # Skip special names and instance properties
                    if property_name.startswith('__') or (property_name.startswith('_') and 
                                                         not property_name.startswith('_' + class_name.lower())):
                        continue
                    ...
                    if assignment_node:
                        ...
                        if expr_node:
                            ...
                            property_value = ""
                            value_node = None
                            for capture_node, capture_name in static_prop_captures:
                                if capture_name == 'prop_value' and capture_node.parent == assignment_node:
                                    value_node = capture_node
                                    break
                            ...
                            property_element = CodeElement(
                                ...
                                name=property_name,
                                content=content,
                                range=CodeRange(start_line=start_line, end_line=end_line, node=expr_node),
                                parent_name=class_name,
            ...
            for match in property_matches:
                ...
                property_element = CodeElement(type=CodeElementType.STATIC_PROPERTY, name=property_name, content=content, 
                                              range=CodeRange(start_line=start_line, end_line=start_line, node=None), 
                                              parent_name=class_name, additional_data={'value': property_value, 'is_static': True})
                class_element.children.append(property_element)
    ...
    def _add_decorator_meta_elements(self, parent_element: CodeElement, decorators: List[str], parent_name: str, target_type: str, target_name: str, class_name: Optional[str]=None) -> None:
        ...
        for decorator in decorators:
            if isinstance(decorator, str):
                ...
                meta_element = CodeElement(type=CodeElementType.DECORATOR, name=decorator_name, content=decorator, 
                                        parent_name=parent_name, additional_data=meta_data)
