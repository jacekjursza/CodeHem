...
class PythonExtractionPostProcessor(BaseExtractionPostProcessor):
    ...
    def process_imports(self, raw_imports: List[Dict]) -> List[CodeElement]:
        ...
        if not raw_imports:
            logger.debug('ProcessImports: No raw imports received, returning empty list.')
            return []
        ...
        try:
            ...
            logger.debug(f'ProcessImports: Sorted {len(valid_imports)} valid imports by line.')
        except Exception as e:
            logger.error(f'ProcessImports: Error sorting valid_imports: {e}. Proceeding without sorting.', exc_info=True)
            # Fallback to original list if sorting fails critically
            valid_imports = [imp for imp in raw_imports if isinstance(imp, dict) and 'range' in imp]
            if not valid_imports: return []
        ...
        if isinstance(start_line, int) and isinstance(end_line, int) and start_line > 0 and end_line >= start_line:
             try:
                 ...
                 logger.debug(f'ProcessImports: Created combined CodeRange: {combined_range}')
             except (ValidationError, Exception) as e:
                 logger.error(f'ProcessImports: Failed to create combined CodeRange: {e}', exc_info=True)
        else:
             logger.warning(f'ProcessImports: Invalid calculated lines for combined range: start={start_line}, end={end_line}. Range will be None.')

        # Combine content (ensure content exists and is string)
        combined_content_parts = [imp.get('content', '') for imp in valid_imports if isinstance(imp.get('content'), str)]
        combined_content = '\n'.join(combined_content_parts)

        logger.debug(f'ProcessImports: Combining {len(valid_imports)} imports from estimated line {start_line} to {end_line}.')
        ...
        try:
            ...
            combined_element = CodeElement(
                ...
                additional_data={'individual_imports': valid_imports} # Keep raw data if needed
            )
            logger.debug("ProcessImports: Successfully created combined 'imports' CodeElement.")
            return [combined_element]
        except (ValidationError, Exception) as e:
            logger.error(f"ProcessImports: Failed to create combined 'imports' CodeElement: {e}", exc_info=True)
    ...
    def process_classes(self, raw_classes: List[Dict], members: List[Dict], static_props: List[Dict]) -> List[CodeElement]:
        ...
        for class_data in raw_classes:
            ...
            if not class_name:
                ...
                # Create a placeholder error element if needed, or just skip
                # class_element = CodeElement(type=CodeElementType.CLASS, name='_ERROR_NO_CLASS_NAME_', content=class_data.get('content', ''))
                # processed_classes.append(class_element)
                continue

            class_data['type'] = CodeElementType.CLASS.value # Ensure type is set
            try:
                class_element = CodeElement.from_dict(class_data)
                class_element.parent_name = None # Classes are top-level in Python modules

                # Process decorators for the class itself
                class_element.children.extend(self._process_decorators(class_data))

                # Process members (methods, properties)
                ...
                processed_members = {} # Use dict to handle potential duplicates from extractor
                for member_data in members_for_this_class:
                    ...
                    if processed_member:
                        # Use a key that includes type to differentiate getter/setter
                ...
                processed_static_props = {} # Use dict to handle potential duplicates
    ...
    def _process_parameters(self, element: 'CodeElement', params_data: List[Dict]) -> List['CodeElement']:
        ...
        for param in params_data:
            if not isinstance(param, dict):
                logger.warning(f"Skipping non-dict item in params_data for {parent_path}: {type(param)}")
                continue
            name = param.get('name')
            if name:
                try:
                    ...
                    logger.error(f"Failed to create parameter CodeElement for '{name}' in {parent_path}: {e}", exc_info=True)
            else:
                 logger.warning(f"Skipping parameter data without a name for {parent_path}: {param}")
    ...
    def _process_return_value(self, element: 'CodeElement', return_info: Dict) -> Optional['CodeElement']:
        if not isinstance(return_info, dict):
             logger.warning(f"Invalid return_info format for {element.name}: {type(return_info)}")
             return None
        ...
        try:
            return_element = CodeElement(
                ...
                additional_data={'values': return_values} # Store observed return values
                # Range usually not applicable here
            )
            return return_element
        except (ValidationError, Exception) as e:
             logger.error(f"Failed to create return value CodeElement for {element.name}: {e}", exc_info=True)
             return None
    ...
    def _process_method_element(self, method_data: Dict, parent_class_element: 'CodeElement') -> Optional['CodeElement']:
        ...
        try:
            ...
            method_data['type'] = element_type_enum.value # Ensure data has valid type string
            ...
            logger.error(f"Failed to process method element '{element_name}' in class '{parent_name}': {e}. Data: {method_data}", exc_info=True)
            return None
    ...
    def _process_static_property(self, prop_data: Dict, parent_class_element: 'CodeElement') -> Optional['CodeElement']:
        ...
        if not isinstance(prop_data, dict):
            logger.warning(f"Skipping non-dict item in static_props data for {parent_name}: {type(prop_data)}")
            return None
        ...
        try:
            ...
            logger.error(f"Failed to process static property '{prop_name}' in class '{parent_name}': {e}. Data: {prop_data}", exc_info=True)
            return None
    ...
    def _process_decorators(self, element_data: Dict) -> List['CodeElement']:
        ...
        if not isinstance(decorators_raw, list):
            logger.warning(f"Invalid decorators format for {full_parent_name}: Expected list, got {type(decorators_raw)}")
            return []

        for dec_data in decorators_raw:
            if not isinstance(dec_data, dict):
                logger.warning(f'Skipping invalid decorator data format for {full_parent_name}: {dec_data}')
                continue
            ...
            if name and content:
                ...
                if isinstance(range_data, dict):
                    try:
                        ...
                        if isinstance(start_line, int) and isinstance(end_line, int) and start_line > 0 and end_line >= start_line:
                             ...
                             logger.warning(f"Invalid line numbers for decorator '{name}' range: start={start_line}, end={end_line}")

                    except (ValidationError, KeyError, Exception) as e:
                        logger.warning(f"Error creating CodeRange for decorator '{name}': {e}. Range data: {range_data}", exc_info=False)
                elif range_data is not None:
                    logger.warning(f"Invalid range format for decorator '{name}': {type(range_data)}")
                ...
                try:
                    ...
                    decorator_elements.append(decorator_element)
                     ...
                     logger.error(f"Failed to create decorator CodeElement for '{name}' in {full_parent_name}: {e}", exc_info=True)
