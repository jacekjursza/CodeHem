...
class TypeScriptStrategy(LanguageStrategy):
    ...
    def is_class_definition(self, line: str) -> bool:
        ...
        """
        return bool(re.match(r'^\s*(export\s+)?(abstract\s+)?class\s+[A-Za-z_][A-Za-z0-9_]*', line))
    ...
    def is_function_definition(self, line: str) -> bool:
        ...
        """
        return bool(re.match(r'^\s*(export\s+)?(function\s+|async\s+function\s+)[A-Za-z_][A-Za-z0-9_]*\s*\(', line))
    ...
    def is_method_definition(self, line: str) -> bool:
        ...
        return bool(re.match(r'^\s*(public|private|protected|static|async)?\s*[A-Za-z_][A-Za-z0-9_]*\s*\(', line))
    ...
    def extract_method_name(self, method_line: str) -> Optional[str]:
        ...
        """
        match = re.match(r'^\s*(public|private|protected|static|async)?\s*([A-Za-z_][A-Za-z0-9_]*)\s*\(', method_line)
    ...
    def extract_class_name(self, class_line: str) -> Optional[str]:
        ...
        """
        match = re.match(r'^\s*(export\s+)?(abstract\s+)?class\s+([A-Za-z_][A-Za-z0-9_]*)', class_line)
    ...
    def extract_function_name(self, function_line: str) -> Optional[str]:
        ...
        """
        match = re.match(r'^\s*(export\s+)?(function\s+|async\s+function\s+)([A-Za-z_][A-Za-z0-9_]*)\s*\(', function_line)
        return match.group(3) if match else None
    ...
    def fix_special_characters(self, content: str, xpath: str) -> Tuple[str, str]:
        ...
        if content:
            ...
            # Also fix method definitions in classes
            method_pattern = r'(public|private|protected|static|async)?\s*\*+([A-Za-z_][A-Za-z0-9_]*)\*+\s*\('
            method_replacement = r'\1 \2('
