...
try:
    ...
    try:
        import_results = ast_handler.execute_query(
            import_query_str, root_node, code_bytes
        )
        ...
        for node, capture_name in import_results:
            ...
            print(
                f"    Node Text: {ast_handler.get_node_text(node, code_bytes).strip()}"
            )
    ...
    logger.debug(f"Query:\n{static_prop_query_str}")
    try:
        static_prop_results = ast_handler.execute_query(
            static_prop_query_str, root_node, code_bytes
        )
        # The query now captures the block node with @class_block
        logger.info(
            f"Static Property Query Results (captures @class_block): {len(static_prop_results)}"
        )
        ...
        for (
            block_node,
            capture_name,
        ) in static_prop_results:  # Should only capture @class_block
            if capture_name == "class_block":
                ...
                for child_node in block_node.named_children:
                    ...
                    prop_name_text = "N/A"
                    prop_value_text = "N/A"
                    prop_type_text = "N/A"  # Default N/A
                    ...
                    if child_node.type == "assignment":
                        ...
                        if inner_node.type == "assignment":
                            ...
                            print(
                                "      (Assignment found within expression_statement)"
                            )
                        # If inner node is typed_assignment (less likely at class level based on logs, but good to check)
                            ...
                            assignment_node = inner_node
                            print(
                                "      (Typed_assignment found within expression_statement - less common case)"
                            )
        ...
        logger.error(
            f"Static Property Query FAILED with unexpected error: {e}", exc_info=True
        )
